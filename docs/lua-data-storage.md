...
已提供异步导入导出、跨卷查询和清理；活动库自动轮转仍在接线，当前不是完整长期记录版本。
...
数据表底部下载图标可选择 CSV 或 PSREC，并通过保存对话框选择文件。
历史表导出点击时固定快照中的全部匹配记录；实时表导出点击事件处理时有界缓存内的全部匹配行，
两者均保留当前筛选和排序、忽略分页。文件选择期间继续采样、翻页或刷新，不改变已冻结的导出内容。
导出保存完整类型化记录，不按显示列隐藏、单位或精度裁剪原始字段，CSV 使用工具类型化格式。

表格单独显示导出中、成功行数、取消或错误状态；导出旁的取消按钮不影响正在进行的历史查询。
保存对话框可直接取消。内部导出和文件选择事件不进入脚本 `on_record/on_file_dialog/on_control`。
任务在存储后台执行，复用文件路径授权、文件大小限额、临时文件原子替换；
失败或取消不覆盖旧文件，不写入历史库，也不改变录制计数。
实时导出最多 1000 行，后台实时快照合计受 `queue_bytes` 限制。
禁用/只读控件、文件 I/O 禁用、授权失败或旧协议代次事件均不能启动导出。
...
容量清理以配置上限的 90% 为目标，预留短查询占用卷期间的写入空间；
暂时无法达到该目标不算容量故障，实际总量超过配置上限且无可清理卷时才故障。
...
`interrupted_from_us/interrupted_to_us` 报告最近记录故障中被拒绝/失败记录的接收时间范围。
未发生失败记录时中断字段为空；范围不是设备端丢包区间。后台提交后的故障范围可跨重启恢复，
突然退出前尚未提交的故障元数据不保证恢复。
`session_id` 在主动停止后重新启动时递增，`run_id` 随数据会话重新打开递增（包括协议重载）。
`unclean_recovery` 表示本次打开前没有已提交的正常退出标记，
`abnormal_runs` 保留异常退出次数；后续正常退出不会抹掉该计数。
...
记录在批次提交前检查 UTC 跨日和 `max_volume_bytes`（默认 256MiB，含 WAL），
非空活动卷达到条件后封存，新卷继续使用全局记录 ID；单批次可能使卷短时超过阈值。
空卷不因大小反复轮转，模式变化和恢复未停止的录制任务会开启新卷。
分卷固定路径保留旧查询快照，旧 `records.sqlite` 在启动时验证身份后可恢复迁移为封存卷。
切换意图、活动指针和会话状态持久化于目录索引；损坏或无法验证的准备卷明确报错，
不会自动删除未知文件。完整长期运行和故障注入验收仍在实施，
软件进程骤停后的 WAL 恢复测试不等同于断电或硬件落盘保证。
