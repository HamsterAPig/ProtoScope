...
- 可选的 `performance.adaptive.enabled` 默认关闭。开启后用 `performance.adaptive.max_multiplier` 作为 K（`0.25..4.0`），按系统 CPU/内存和应用 backlog 动态接管 FPS、波形渲染、UI backlog 追赶和脚本输出 flush 预算；`performance.scale` 与这些热调项不参与运行时调度。
