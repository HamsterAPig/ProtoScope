...
- `波形 Dock`：支持 Lua 推送多通道波形、总览、测量、FFT、原始波形导入、当前缓存快照导出和完整原始数据录制
...
- 源码默认示例目录为 `protocols/default_protocol`，打包运行时会释放到可执行目录下供程序加载
...
源码默认脚本 `protocols/default_protocol/main.lua` 已是可运行示例；打包运行时会释放到可执行目录下供程序加载。它覆盖了：
...
| selected_dir | string | protocols/templates/default_protocol | 当前选中的运行时协议目录；源码默认示例位于 protocols/default_protocol |
