...
local function active_frames(seq)
  local out = {}
  for _, row in ipairs(seq.frames or {}) do
    if row.enabled then out[#out + 1] = row end
  end
  return out
end
...
local function send_next()
  ...
  local rows = active_frames(seq)
  if #rows == 0 then return end
  if cursor > #rows then cursor = 1 end
  proto.send(build_frame(rows[cursor]), { tag = rows[cursor].name })
  cursor = cursor + 1
  if cursor <= #rows or seq.loop then
...
function on_timer(ctx, name)
  if name == timer_name then send_next() end
end
```
