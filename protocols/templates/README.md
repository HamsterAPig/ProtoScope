...
- `ui_layouts`：适合整理较多协议参数。演示 `column`、`flow`、`inline_group`、`table`、`group`、`collapse` 和控件宽度约束。
- `ui_dialogs`：适合脚本需要提示或二次确认的场景。演示 `proto.ui.alert()`、`proto.ui.confirm()`、`window` 参数和 `on_dialog()`。
