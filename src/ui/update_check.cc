...
namespace {
    ...
    bool httpGetGitHubTags(std::string& responseBody, std::string& error)
    {
        WinHttpHandle session(WinHttpOpen(
            L"ProtoScope/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0));
        if (!session.value) {
        ...
        if (!WinHttpSendRequest(request.value, kHeaders, static_cast<DWORD>(-1L), WINHTTP_NO_REQUEST_DATA, 0, 0, 0) ||
...
UpdateCheckResult evaluateUpdateCheckTags(std::string_view responseBody, BuildVersionInfo buildInfo)
{
    ...
    const std::string baseTag =
        buildInfo.baseTag.empty() ? toString(buildInfo.currentTag) : toString(buildInfo.baseTag);
...
#else
    return {UpdateCheckResult::State::Unsupported, "暂不支持检查更新", "当前平台暂未实现联网检查更新", ""};
#endif
}
