...
namespace {
    ...
    bool beginEditableComboPopup(const char* label, const char* previewValue, bool& enterEditMode)
    {
        ...
        const ImRect totalBb(
            bb.Min, ImVec2(bb.Max.x + (labelSize.x > 0.0f ? style.ItemInnerSpacing.x + labelSize.x : 0.0f), bb.Max.y));
        ...
        window->DrawList->AddRectFilled(
            bb.Min, ImVec2(valueX2, bb.Max.y), frameColor, style.FrameRounding, ImDrawFlags_RoundCornersLeft);
        ...
        if (labelSize.x > 0.0f) {
            ImGui::RenderText(
                ImVec2(bb.Max.x + style.ItemInnerSpacing.x, bb.Min.y + style.FramePadding.y), label, labelEnd, false);
        }
                                      ...
                                      const std::function<bool(const std::string&)>& validator)
{
    ...
    if (editing) {
        ...
        if (comboOptions.keepPopupOpenWhileEditing && storage->GetBool(editingKey, false)) {
            ...
            bool mouseDownInPopup = false;
            bool selectedFromPopup = false;
            if (ImGui::BeginComboPopup(popupId, inputBb, ImGuiComboFlags_None)) {
                ...
                const ImRect popupBb(popupPos, ImVec2(popupPos.x + popupSize.x, popupPos.y + popupSize.y));
                mouseDownInPopup = ImGui::IsMouseDown(ImGuiMouseButton_Left) && popupBb.Contains(ImGui::GetMousePos());
            ...
            if (inputDeactivated && storage->GetBool(editingKey, false)) {
                // 核心流程：候选弹层内的首个鼠标点击要留给 Selectable 处理，不能先让输入框失焦关闭编辑态。
                if (storage->GetBool(suppressFirstDeactivateKey, false)) {
                    storage->SetBool(suppressFirstDeactivateKey, false);
                } else if (!mouseDownInPopup && !selectedFromPopup) {
                    storage->SetBool(editingKey, false);
                }
            }
            if (storage->GetBool(editingKey, false) && !inputActive && !ImGui::IsAnyItemActive() &&
