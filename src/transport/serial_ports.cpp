...
std::vector<std::string> queryWindowsSerialPorts() {
    HKEY key = nullptr;
    const LONG openResult = RegOpenKeyExA(
        HKEY_LOCAL_MACHINE,
    ...
    for (DWORD index = 0;; ++index) {
        ...
        DWORD valueType = 0;
        const LONG enumResult = RegEnumValueA(
            key,
            index,
            valueName,
            &valueNameSize,
...
#else
void appendMatchingDevices(std::vector<std::string>& ports, const std::filesystem::path& directory, std::string_view prefix) {
