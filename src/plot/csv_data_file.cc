...
std::string encodePlotSetupRecordHex(const RawCaptureEvent& event)
{
    ...
    return encodeHex(std::span<const std::uint8_t>(reinterpret_cast<const std::uint8_t*>(text.data()), text.size()));
}
...
std::optional<WaveCsvData> readWaveCsvFile(const std::filesystem::path& path, std::string& error)
{
    ...
    if (const auto frequencyText = metadataValue(parsed->metadata, "sample_frequency_hz"); !frequencyText.empty()) {
    ...
    if (data.shape == WaveCsvShape::Long || columns.contains("channel_index")) {
        ...
        for (std::size_t rowIndex = 1; rowIndex < parsed->rows.size(); ++rowIndex) {
            ...
            if (!parseDouble(cell(row, timeColumn->second), time) || !parseDouble(cell(row, valueColumn->second), value)) {
    ...
    for (std::size_t channelIndex = 0; channelIndex < data.channels.size(); ++channelIndex) {
        ...
        std::sort(channel.samples.begin(), channel.samples.end(), [](const WaveSample& left, const WaveSample& right) {
                            ...
                            std::string& error)
{
    try {
        ...
        const auto resolvedRange = resolveCsvExportTimeRange(range);
        const auto events = capture.events.empty()
                                ? std::vector<RawCaptureEvent>{RawCaptureEvent{
                                      .type = RawCaptureEventType::RxBytes,
...
std::optional<RawCaptureFileData> readRawCaptureCsvFile(const std::filesystem::path& path, std::string& error)
{
    ...
    for (std::size_t rowIndex = 1; rowIndex < parsed->rows.size(); ++rowIndex) {
        ...
        if (!parseRawEventType(cell(row, eventTypeColumn->second), event.type)) {
            error = "CSV 第 " + std::to_string(row.line) + " 行 event_type 未知";
            return std::nullopt;
        }
        if (!parseUnsigned(cell(row, timestampColumn->second), event.timestampMs)) {
        ...
        if (event.type == RawCaptureEventType::RxBytes) {
            ...
            if (bytesColumn == columns.end() || !decodeHexBytes(cell(row, bytesColumn->second), event.bytes, row.line, error)) {
                return std::nullopt;
            }
        } else if (event.type == RawCaptureEventType::ProfileSet) {
            ...
            if (const auto mapColumn = columns.find("profile_channel_map"); mapColumn != columns.end()) {
                if (!parseChannelMapSemicolon(cell(row, mapColumn->second), event.profile.channelMap, row.line, error)) {
                    return std::nullopt;
                }
            }
        } else if (event.type == RawCaptureEventType::ProfileClear) {
            ...
            event.timestampMs = [&]() {
                ...
                return parseUnsigned(cell(row, timestampColumn->second), timestamp) ? timestamp : event.timestampMs;
            }();
        }
